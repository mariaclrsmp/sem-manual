...
const BEDROOM_FULL: StarterPackItem[] = [
  ...
  { emoji: '🪴', label: 'Planta de interior facil' },
  { emoji: '🔧', label: 'Kit ferramentas basico' },
];
...
const PET_ITEMS: StarterPackItem[] = [
  ...
  { emoji: '🏥', label: 'Carteira de vacinacao em dia' },
];
...
function PackCard({ profile, sections }: { profile: UserProfile; sections: StarterPackSection[] }) {
  return (
...
export default function StarterPackScreen() {
  ...
  async function handleShare() {
    try {
      ...
      if (!canShare) {
        Alert.alert('Compartilhamento indisponivel', 'Seu dispositivo nao suporta compartilhamento.');
        return;
      }
      await Sharing.shareAsync(uri, { mimeType: 'image/png', dialogTitle: 'Compartilhar meu Starter Pack' });
    } catch {
      Alert.alert('Erro', 'Nao foi possivel capturar o card.');
    }
  }
    ...
    <View style={{ flex: 1, backgroundColor: colors.fundo }}>
      ...
      <SafeAreaView edges={['top']} style={{ backgroundColor: colors.verde }}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerSub}>Tudo que voce precisa para comecar</Text>
        </View>
      </SafeAreaView>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <Pressable onPress={() => router.replace('/(tabs)')} style={styles.continueButton}>
          <Text style={styles.continueButtonText}>Comecar a usar o app</Text>
          <ArrowRight size={18} color={colors.verde} />
        </Pressable>
      </ScrollView>
    </View>
  );
...
const styles = StyleSheet.create({
  header: { paddingHorizontal: 20, paddingTop: 8, paddingBottom: 20, gap: 2 },
  headerTitle: { fontFamily: fonts.extrabold, fontSize: 22, color: '#fff' },
  headerSub: { fontFamily: fonts.regular, fontSize: 13, color: 'rgba(255,255,255,0.8)' },
  ...
  cardHeader: { backgroundColor: colors.verde, paddingHorizontal: 24, paddingVertical: 20 },
  cardTitle: { fontFamily: fonts.extrabold, fontSize: 26, color: '#fff' },
  cardSubtitle: { fontFamily: fonts.bold, fontSize: 18, color: 'rgba(255,255,255,0.9)', marginTop: 2 },
  section: { paddingHorizontal: 20, paddingTop: 16, paddingBottom: 4, gap: 8 },
  sectionTitle: { fontFamily: fonts.extrabold, fontSize: 14, color: colors.texto },
  itemsGrid: { gap: 6 },
  item: { flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 4, paddingHorizontal: 4 },
  itemEmoji: { fontSize: 18, width: 26, textAlign: 'center' },
  itemLabel: { fontFamily: fonts.regular, fontSize: 14, color: colors.texto, flex: 1 },
  cardFooter: { paddingHorizontal: 20, paddingBottom: 20, paddingTop: 12, alignItems: 'center', gap: 10 },
  footerDivider: { width: '100%', height: 1, backgroundColor: '#E5E7EB' },
  footerText: { fontFamily: fonts.semibold, fontSize: 11, color: '#9CA3AF', letterSpacing: 0.4 },
  shareButton: {
  ...
  shareButtonText: { fontFamily: fonts.extrabold, fontSize: 16, color: '#fff' },
  continueButton: {
    borderRadius: 16,
    paddingVertical: 16,
    alignItems: 'center',
  ...
  continueButtonText: { fontFamily: fonts.bold, fontSize: 16, color: colors.verde },
});
